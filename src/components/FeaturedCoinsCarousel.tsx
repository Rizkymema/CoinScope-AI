...
export const FeaturedCoinsCarousel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchBoosted = async () => {
      // First try boosted tokens (they have images), fallback to top coins
    ...
    return (
      <div className="w-full mb-12">
        <div className="flex items-center justify-between mb-6">
          <div>
            <div className="h-7 w-48 bg-slate-800 rounded-lg animate-pulse"></div>
            <div className="h-4 w-32 bg-slate-800/50 rounded-lg animate-pulse mt-2"></div>
          </div>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          {[1,2,3,4,5].map(i => (
            <div key={i} className="bg-slate-900/80 backdrop-blur-sm border border-slate-800/50 rounded-2xl p-4 animate-pulse">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-12 h-12 rounded-full bg-slate-800"></div>
                <div className="flex-1">
                  <div className="h-4 w-16 bg-slate-800 rounded mb-1"></div>
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
            ...
            {Array.from({ length: Math.max(1, coins.length - visibleCount + 1) }).slice(0, 10).map((_, i) => (
              <button
                ...
                className={`w-1.5 h-1.5 rounded-full transition-all duration-300 ${
                  currentIndex === i ? 'bg-amber-400 w-4' : 'bg-slate-700 hover:bg-slate-600'
                }`}
              />
            ))}
          </div>
          <button
            ...
            className="p-2 hover:bg-slate-800 rounded-xl transition-all border border-slate-700/50 disabled:opacity-30 disabled:cursor-not-allowed"
          >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 overflow-hidden">
        {visibleCoins.map((coin, idx) => (
          <div
            ...
            className="group relative bg-slate-900/80 backdrop-blur-sm border border-slate-800/50 rounded-2xl p-4 hover:border-amber-500/30 transition-all duration-300 cursor-pointer hover:-translate-y-1 hover:shadow-lg hover:shadow-amber-500/5"
          >
            {/* Subtle gradient overlay */}
            <div className="absolute inset-0 rounded-2xl bg-gradient-to-b from-amber-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
